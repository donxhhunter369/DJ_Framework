...
#import "NSObject+DJJSON.h"

@implementation NSObject (DJJSON)
...
+ (id)objectFromString:(id)str
{
    ...
    else if ( [DJTypeEncoding isAtomClass:[obj class]] )
    {
        return obj;
    }
...
- (id)objectToDictionaryUntilRootClass:(Class)rootClass
{
    ...
    if ( [self isKindOfClass:[NSDictionary class]] )
    {
        ...
        for ( NSString * key in dict.allKeys )
        {
            ...
            if ( obj )
            {
                NSUInteger propertyType = [DJTypeEncoding typeOfObject:obj];
                if ( DJTypeEncoding.NSNUMBER == propertyType )
                {
                    [result setObject:obj forKey:key];
                }
                else if ( DJTypeEncoding.NSSTRING == propertyType )
                {
                    [result setObject:obj forKey:key];
                }
                else if ( DJTypeEncoding.NSARRAY == propertyType )
                {
                    NSMutableArray * array = [NSMutableArray array];
                    ...
                    for ( NSObject * elem in (NSArray *)obj )
                    {
                        ...
                        {
                            if ( [DJTypeEncoding isAtomClass:[elem class]] )
                            {
                ...
                else if ( DJTypeEncoding.NSDICTIONARY == propertyType )
                {
                    ...
                    for ( NSString * key in ((NSDictionary *)obj).allKeys )
                    {
                        ...
                        if ( val )
                        {
                            ...
                            {
                                if ( [DJTypeEncoding isAtomClass:[val class]] )
                                {
                ...
                else if ( DJTypeEncoding.NSDATE == propertyType )
                {
    ...
    else
    {
        for ( Class clazzType = [self class];; )
        {
            ...
            {
                if ( [DJTypeEncoding isAtomClass:clazzType] )
                    break;
            }
            ...
            for ( NSUInteger i = 0; i < propertyCount; i++ )
            {
                ...
                NSUInteger		propertyType = [DJTypeEncoding typeOf:attr];
                ...
                if ( obj )
                {
                    ...
                    else if ( DJTypeEncoding.NSARRAY == propertyType )
                    {
                        ...
                        for ( NSObject * elem in (NSArray *)obj )
                        {
                            NSUInteger elemType = [DJTypeEncoding typeOfObject:elem];
                            
                            if ( DJTypeEncoding.NSNUMBER == elemType )
                            {
                                [array addObject:elem];
                            }
                            else if ( DJTypeEncoding.NSSTRING == elemType )
                            {
                                [array addObject:elem];
                            }
                            else
...
- (id)objectZerolizeUntilRootClass:(Class)rootClass
{
    for ( Class clazzType = [self class];; )
    {
        ...
        for ( NSUInteger i = 0; i < propertyCount; i++ )
        {
            ...
            NSString *		propertyName = [NSString stringWithCString:name encoding:NSUTF8StringEncoding];
            NSUInteger		propertyType = [DJTypeEncoding typeOfAttribute:attr];
            ...
            else if ( DJTypeEncoding.OBJECT == propertyType )
            {
                Class clazz = [DJTypeEncoding classOfAttribute:attr];
                if ( clazz )
                {
...
{
    NSString *	json = nil;
    NSUInteger	propertyType = [DJTypeEncoding typeOfObject:self];
...
- (id)serializeObject
{
    NSUInteger type = [DJTypeEncoding typeOfObject:self];
    
    if ( DJTypeEncoding.NSNUMBER == type )
    {
        return self;
    }
    else if ( DJTypeEncoding.NSSTRING == type )
    {
        return self;
    }
    else if ( DJTypeEncoding.NSDATE == type )
    {
        return self;
    }
    else if ( DJTypeEncoding.NSARRAY == type )
    {
    ...
    else if ( DJTypeEncoding.NSDICTIONARY == type )
    {
        NSDictionary *			dict = (NSDictionary *)self;
        NSMutableDictionary *	result = [NSMutableDictionary dictionary];
    ...
    else if ( DJTypeEncoding.OBJECT == type )
    {
...
{
    NSUInteger type = [DJTypeEncoding typeOfObject:obj];
